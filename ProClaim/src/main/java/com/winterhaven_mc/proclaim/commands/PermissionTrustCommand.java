...
final class PermissionTrustCommand implements CommandExecutor, TabCompleter {
	 ...
	 */
	PermissionTrustCommand(final PluginMain plugin) {
		
		// set reference to main class
		this.plugin = plugin;
		
		// register this class as command executor
		plugin.getCommand("permissiontrust").setExecutor(this);
	}
	...
	public final boolean onCommand(final CommandSender sender, final Command command, 
		...
		if (!playerState.isAdminMode() && !claim.getOwnerUUID().equals(playerState.getPlayerUUID())) {
			...
			plugin.messageManager.sendPlayerMessage(sender, "COMMAND_FAIL_TRUST_NOT_OWNER",claim);
			return true;
		}
		...
		// get player current permission level for claim
		final ClaimPermission recipientClaimPermission = 
				ClaimPermission.getClaimPermission(claim.getKey(), recipientUUID);
		
		// if no claim permission for player, set access + grant
		if (recipientClaimPermission == null) {
			claim.setPermission(recipientUUID, PermissionLevel.ACCESS_GRANT);
		}
		else {
			// add grant permission to existing permission level
			PermissionLevel newPermissionLevel = recipientClaimPermission.addGrant();
			claim.setPermission(recipientUUID, newPermissionLevel);
		}
		
		// send player success message
		plugin.messageManager.sendPlayerMessage(sender, "COMMAND_SUCCESS_GRANT_TRUST",claim);
