...
final class TrustCommand implements CommandExecutor, TabCompleter {
	 ...
	 */
	TrustCommand(final PluginMain plugin) {
	...
	public final List<String> onTabComplete(final CommandSender sender, final Command command, 
		...
		if (args.length == 1) {
			...
			List<Player> matchingPlayers = plugin.getServer().matchPlayer(args[0]);
	...
	public final boolean onCommand(final CommandSender sender, final Command command, 
		...
		if (!playerState.isAdminMode() 
				&& !claim.getOwnerUUID().equals(playerState.getPlayerUUID())
				&& !claimPermission.canGrant(PermissionLevel.BUILD)) {
			
			// send grant build permission fail message and return
			plugin.messageManager.sendPlayerMessage(sender, "COMMAND_FAIL_GRANT_BUILD_TRUST",claim);
			return true;
		}
		...
		// get recipient current claim permission
		final ClaimPermission recipientClaimPermission = 
				ClaimPermission.getClaimPermission(claim.getKey(), recipientUUID);
		
		// if recipient has grant permission, set permission level to build + grant
		if (recipientClaimPermission != null && recipientClaimPermission.allows(PermissionLevel.GRANT)) {
			claim.setPermission(recipientUUID, PermissionLevel.BUILD_GRANT);
		}
		// else set permission level to build
		else {
			claim.setPermission(recipientUUID, PermissionLevel.BUILD);
		}
		
		// send player success message
		plugin.messageManager.sendPlayerMessage(sender, "COMMAND_SUCCESS_TRUST",claim);
