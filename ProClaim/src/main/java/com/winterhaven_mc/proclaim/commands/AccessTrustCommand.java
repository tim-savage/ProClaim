...
final class AccessTrustCommand implements CommandExecutor, TabCompleter {
	 ...
	 */
	AccessTrustCommand(final PluginMain plugin) {
		
		// set reference to main class
		this.plugin = plugin;
		
		// register this class as command executor
		plugin.getCommand("accesstrust").setExecutor(this);
	}
	...
	public final boolean onCommand(final CommandSender sender, final Command command, 
		...
		// get player permission record for claim
		...
		if (!playerState.isAdminMode() && !claim.getOwnerUUID().equals(playerState.getPlayerUUID())
				&& !claimPermission.canGrant(PermissionLevel.ACCESS)) {
			
			// send not owner message and return
			plugin.messageManager.sendPlayerMessage(sender, "COMMAND_FAIL_GRANT_ACCESS_TRUST",claim);
			return true;
		}
		...
		// if recipient has grant permission, set permission level to access + grant
		...
		// else set permission level to access
		else {
			claim.setPermission(recipientUUID, PermissionLevel.ACCESS);
		}
		
		// send player success message
		plugin.messageManager.sendPlayerMessage(sender, "COMMAND_SUCCESS_ACCESS_TRUST",claim);
